...
static void compute_layer_outputs(double *, size_t, double const *, size_t,
                                  double **);
static double sigmoid(double);
static double inner_product(double const *, double const *, size_t);
static void update_weights(double **, size_t, double const *, size_t,
                           double const *, double);
...
void train_mlp(Mlp3 *mlp, Dataset const *dataset) {
  ...
  do {
    ...
    for (size_t i = 0; i < dataset->vector_count; i++) {
      ...
      update_weights(mlp->output_weights, mlp->output_neuron_count,
                     mlp->hidden_outputs, mlp->hidden_neuron_count,
                     mlp->output_errors, mlp->learning_rate);
      update_weights(mlp->hidden_weights, mlp->hidden_neuron_count,
                     input_vector->elements, mlp->input_neuron_count,
                     mlp->hidden_errors, mlp->learning_rate);
    }
...
static void randomize_weights(Mlp3 *mlp) {
  for (size_t i = 0;
       i < mlp->hidden_neuron_count * (mlp->input_neuron_count + 1); i++) {
    mlp->hidden_weights[0][i] = frand_xorshift();
  }

  for (size_t i = 0;
       i < mlp->output_neuron_count * (mlp->hidden_neuron_count + 1); i++) {
...
void compute_outputs(Mlp3 *mlp, FeatureVector const *input_vector) {
  ...
  compute_layer_outputs(mlp->hidden_outputs, mlp->hidden_neuron_count,
                        input_vector->elements, mlp->input_neuron_count,
                        mlp->hidden_weights);
  compute_layer_outputs(mlp->outputs, mlp->output_neuron_count,
                        mlp->hidden_outputs, mlp->hidden_neuron_count,
                        mlp->output_weights);
}
 ...
 */
static void compute_layer_outputs(double *outputs, size_t output_count,
                                  double const *inputs, size_t input_dimension,
                                  double **weights) {
  for (size_t i = 0; i < output_count; i++) {
    double const *weight = weights[i];

    // The last element of weight is the threshold (bias).
    outputs[i] = sigmoid(inner_product(weight, inputs, input_dimension) +
                         (-1.0 * weight[input_dimension]));
  }
}
...
/**
 * @brief Calculate all errors (neural network errors, output layer errors and
 * hidden layer errors).
 * @param mlp The pointer to the given MLP.
...
double compute_errors(Mlp3 *mlp, TeacherVector const *teacher_vector) {
  ...
  for (size_t i = 0; i < mlp->output_neuron_count; i++) {
    ...
    //          Note that the activation function in this program is `the
    //          unipolar sigmoid function`.
    mlp->output_errors[i] = diff * (output * (1 - output));
 ...
 */
static void update_weights(double **weights, size_t neuron_count,
                           double const *inputs, size_t input_dimension,
                           double const *errors, double learning_rate) {
  for (size_t i = 0; i < neuron_count; i++) {
    for (size_t j = 0; j < input_dimension; j++) {
      // TODO:: 4. Update weight based on backpropagation.
      weights[i][j] += learning_rate * inputs[j] * errors[i];
    }
