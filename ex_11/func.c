...
/**
 * @brief Training Self-organizing neural network based on Winner-take-all
 * learning rule.
...
void train_sonnet(Sonnet *son, Dataset const *dataset) {
  ...
  for (size_t i = 0; i < son->update_limit; i++) {
    for (size_t j = 0; j < dataset->vector_count; j++) {
      ...
      // Step 2. Find the winner neuron which is most similar to the given
      // feature vector.
      size_t winner_index = 0;
      double min_distance = DBL_MAX;
      for (size_t k = 0; k < son->neuron_count; k++) {
        // TODO: 2. Find the most similar prototype using euclidean distance.
        double d = euclidean_distance(feature, son->weight_vectors[k],
                                      feature_dimension);
        if (d < min_distance) {
          min_distance = d;
          winner_index = k;
      ...
      for (size_t k = 0; k < feature_dimension; k++) {
        ...
        winner_weight[k] +=
            son->learning_rate * (feature[k] - winner_weight[k]);
      }
    }
  }
}
