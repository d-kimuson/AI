...
static void classify_vectors(Dataset const *, size_t *, FeatureVector const *,
                             size_t);
static bool update_reference_pattern(Dataset const *, size_t const *,
                                     FeatureVector *, size_t);
 ...
 * @param predicted_labels The array which contains the classification results
 ...
 */
void do_kmeans(Dataset const *dataset, size_t *predicted_labels,
               FeatureVector *reference_patterns, size_t cluster_count) {
  ...
  update_reference_pattern(dataset, predicted_labels, reference_patterns,
                           cluster_count);

  bool is_almost_equal = false;
  do {
    // Step: 2. Calculate nearest reference pattern for each vector.
    classify_vectors(dataset, predicted_labels, reference_patterns,
                     cluster_count);

    // Step: 3. Update the all reference patterns using mean.
    is_almost_equal = update_reference_pattern(
        dataset, predicted_labels, reference_patterns, cluster_count);

    // Step: 4. If new reference patterns are almost equal, stop iteration.
    // Otherwise, go to Step 2.
  } while (is_almost_equal == false);
}

/**
 * @brief Classify the vectors in the given dataset using the reference
 * patterns.
 * @param dataset The pointer to the given dataset.
 * @param predicted_labels The array which contains the classification results
 * from k-means algorithm.
 ...
 * @param cluster_count The number of clusters.
 */
static void classify_vectors(Dataset const *dataset, size_t *predicted_labels,
                             FeatureVector const *reference_patterns,
  ...
  for (size_t i = 0; i < dataset->feature_count; i++) {
    ...
    for (size_t j = 0; j < cluster_count; j++) {
      ...
      // TODO: 1. Calculate the most similar reference pattern using euclidean
      // distance.
      double distance = euclidean_distance(reference_pattern->elements,
                                           input_vector->elements,
                                           dataset->feature_dimension);
      if (distance < min_distance) {
        // TODO: 2. Update the predicted label of the feature vector if a more
        // similar reference pattern is found.
 ...
 * @param reference_patterns The array of the reference patterns.
 * @param cluster_count The number of clusters.
 * @return true if the difference of the reference patterns was not large,
 * otherwise false.
 */
static bool update_reference_pattern(Dataset const *dataset,
                                     size_t const *predicted_labels,
                                     FeatureVector *reference_patterns,
                                     size_t cluster_count) {
  ...
  for (size_t i = 0; i < cluster_count; i++) {
    ...
    memcpy(&previous_elements[i * feature_dimension],
           reference_pattern->elements, feature_dimension_byte);
  ...
  for (size_t i = 0; i < cluster_count; i++) {
    ...
    double d = euclidean_distance(reference_pattern->elements,
                                  &previous_elements[i * feature_dimension],
                                  dataset->feature_dimension);
    if (EPS < d) {
 ...
 */
static double euclidean_distance(double const *v1, double const *v2,
                                 size_t length) {
  // TODO: 3. Update the predicted label of the feature vector if more similar
  // reference patterns is found.
  // d = Σ(q-p)^2
  double distance = 0;
  int i;
 ...
 * @param cluster_count The number of clusters.
 */
void adjust_labels(Dataset const *dataset, size_t *predicted_labels,
                   FeatureVector *reference_patterns, size_t cluster_count) {
 ...
 * @return Accuracy.
 */
double validate(Dataset const *dataset, size_t *predicted_labels,
                FeatureVector *reference_patterns, size_t cluster_count) {
  classify_vectors(dataset, predicted_labels, reference_patterns,
                   cluster_count);
